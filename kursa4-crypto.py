print("Выберите алгоритм хэширования [224,256,384,512,512256,512224]:")
check = int(input())
while check != 256 or check != 224 or check != 384 or check !=512 or check != 512224 or check != 512256:
    print("введите указанный алгоритм хеширования")
    check = int(input())
    if check == 256 or check == 224 or check == 384 or check ==512 or check == 512224 or check == 512256:
        break
...
if check == 512:
   ...
   for k in range(16, 80):
       # sigma0 = right_rotate(parts[k-15], 1) xor right_rotate(parts[k-15], 8) xor shift_right(parts[k-15], 7)-15], 3)
       rr1 = rotate_right(parts[k - 15], 1)
       rr2 = rotate_right(parts[k - 15], 8)
       sr = shift_right(parts[k - 15], 7)
       x1 = xor(rr1, rr2)
       s0 = xor(x1, sr)
       # sigma1 = right_rotate(parts[k-2], 19) xor right_rotate(parts[k-2], 61) xor shift_right(parts[k-2], 6)2], 10)
